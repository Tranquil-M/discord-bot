...
async def on_app_command_error(
    ...
    if isinstance(error, app_commands.errors.MissingPermissions):
        await interaction.response.send_message(
            f"You don't have the required permissions to use this command. You need: {', '.join(error.missing_permissions)}. Sorry!",
            ephemeral=True,
        )
...
async def get_or_create_audit_channel(
    guild: discord.Guild, name: str = "audit-log"
) -> discord.TextChannel:
    ...
    new_channel = await guild.create_text_channel(
        name, overwrites=overwrites, reason="Creating audit log channel"
    )
    return new_channel
