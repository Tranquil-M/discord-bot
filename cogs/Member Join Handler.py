...
class Member_Join_Handler(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.leaving = "./cogs/leaving_images"
        self.welcoming = "./cogs/welcome_images"
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        images = [image for image in os.listdir(self.welcoming)]
        random_image = random.choice(images)

        bg = easy_pil.Editor(f"{self.welcoming}/{random_image}").resize((1920, 1080))
        ...
        await welcome_channel.send(
            f"Hey, {member.mention}! Please read and follow the rules in the rules channel, thank you!"
        )
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        images = [image for image in os.listdir(self.leaving)]
        file = discord.File(f"{self.leaving}/{random.choice(images)}")
